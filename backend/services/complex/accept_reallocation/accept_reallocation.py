...
def decline_reallocation(reservation_id):
    # Process a reallocation decline similar to cancellation but with different notification type
    try:
    ...
    # Queue a notification message to RabbitMQ with different message type
    try:
        notification_data = {
            ...
            "payment_id": payment_id,
            "message_type": "reservation.reservation.decline"
        }
        
        publish_to_rabbitmq("reservation.reservation.decline", notification_data)
        
        # Trigger reallocation with next user on waitlist
        ...
        return jsonify({
            "message": "Reallocation declined and notification sent. Reallocation triggered.",
            "status": "declined",
            "reservation_id": reservation_id,
            "payment_id": payment_id  
        }), 200
